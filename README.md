...
- **🔐 Sensitive Content Protection**: Mark any snippet as sensitive with the 🔒 button. Sensitive content is masked in card previews and never appears in search-content matching or clipboard duplicate detection; it is shown only after an explicit Reveal action (and in the editor, since opening it is already explicit). The flag is persisted and included in export/import. *No authentication or encryption yet — this controls visibility, not storage security.*
