...
- **🔐 Zero-Knowledge Sensitive Snippet Encryption** *(optional)*: Protect sensitive snippet contents using standard **AES-256-GCM** authenticated encryption with an **Argon2id**-derived master key and unique random 96-bit nonces. No encryption keys or secrets are ever written to disk or the SQLite database. When the vault is locked, sensitive snippets are safely encrypted on disk and masked in the UI with a quick "Unlock" action; when unlocked, they are decrypted in volatile memory for the session. Normal snippets remain completely unaffected.
...
- **⚙️ Centralized Settings & Shortcuts Guide**: Dedicated settings hub featuring an interactive keyboard shortcuts cheat sheet, global hotkey recorder, clipboard retention configurator, vault security panel, and backup import/export.
